...
indexer. `CONFIG_FILE` selects its YAML file and defaults to `config.yaml`. Start from
[`indexer-standalone/config.yaml`](../indexer/indexer-standalone/config.yaml), set writable and
distinct paths for `infra.storage.cnn_url` and `infra.ledger_db.cnn_url`, and provide secrets with
the existing `APP__*_FILE` mechanism.
