...
is disabled unless the node is started with an indexer configuration file:

```shell
midnight-node --embedded-indexer-config /etc/midnight/indexer.yaml
```
...
indexer. Start from [`indexer-standalone/config.yaml`](../indexer/indexer-standalone/config.yaml),
set writable and distinct paths for `infra.storage.cnn_url` and `infra.ledger_db.cnn_url`, and
provide secrets with the existing `APP__*_FILE` mechanism.
