...
**`batches.rs`** may be removed to cut complexity (largest builder, parallel tokio tasks). If removed: delete the file, the `Builder::Batches` variant + `BatchesArgs`, its arms in `relevant_wallet_seeds` / `to_builder_v7` / `to_builder_v8`, and the now-unused `compute_batches_seeds` import.
