...
use subxt::{backend::rpc::RpcClient, ext::{ codec::{Decode, Encode}, subxt_rpcs::{client::{RpcParams, RpcSubscription}, rpc_params}}, OnlineClient, PolkadotConfig};
...
impl Relayer {
    ...
    async fn handle_justification_stream_data(&self, justification: Bytes) {
        ...
        let x = beef_signed_commitment.commitment.payload.get_decoded(id)
        ...
        self.get_mmr_root(block_hash).await;
    ...
    async fn get_mmr_root(&self, block_hash:H256) {
...
fn get_decoded_proof(leaves_proof:&LeavesProof<H256>) -> LeafProof<H256> {
    ...
    let keccak = Keccak256::hash(&encoded_mmr_proof.0);
    let hex_keccak = hex::encode(keccak);
    println!("hex mmr root: {hex_keccak}");
