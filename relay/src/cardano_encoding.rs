...
use pallas::{codec::utils::MaybeIndefArray, ledger::primitives::{BoundedBytes, Constr, PlutusData}};
        ...
        PlutusData::Constr(
            Constr {
                ...
                fields: MaybeIndefArray::Indef(vec![
                    ...
                    PlutusData::BigInt(
                        BigInt::Int((self.authority_index as i64).into())
                    ),
            ...
            Constr {
                ...
                fields: MaybeIndefArray::Indef(vec![
                    PlutusData::Array(MaybeIndefArray::Indef(
                        self.payloads.iter().map(|i| {
                            i.to_plutus_data()
                        }).collect()
                    )),
                    PlutusData::BigInt(
                        BigInt::Int((self.block_number as i64).into())
                    ),
                    PlutusData::BigInt(
                        BigInt::Int((self.validator_set_id as i64).into())
                    ),
                ])
            }
        )
    ...
    pub fn from_signed_commitment_and_validators(
        ...
        let commitment = Commitment {
            payloads: signed_commitment.commitment.payload.get_all_raw(&MMR_ROOT_ID).into_iter().map(|i| {
                Payload {id: MMR_ROOT_ID.to_vec(), data: i.clone() }
            }).collect(),
        ...
        let votes = signed_commitment.signatures.iter().enumerate().zip(validator_set.iter()).filter_map(|((i, sig), pk)| {
            match sig {
                Some(sig) =>{
