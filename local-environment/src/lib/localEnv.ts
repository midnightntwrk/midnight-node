...
export function getLocalEnvSecretVars(): Record<string, string> {
  ...
  if (!process.env.LOCALENV_POSTGRES_PASSWORD) {
    result.LOCALENV_POSTGRES_PASSWORD = fs.readFileSync('localenv_postgres.password', 'utf-8').trim();
  }

  if (!process.env.APP__INFRA__STORAGE__PASSWORD) {
    result.APP__INFRA__STORAGE__PASSWORD = fs.readFileSync('localenv_app_storage.password', 'utf-8').trim();
  }

  if (!process.env.APP__INFRA__PUB_SUB__PASSWORD) {
    result.APP__INFRA__PUB_SUB__PASSWORD = fs.readFileSync('localenv_pubsub.password', 'utf-8').trim();
  }

  if (!process.env.APP__INFRA__SECRET) {
    result.APP__INFRA__SECRET = fs.readFileSync('localenv_app_infra_secret.password', 'utf-8').trim();
  }
