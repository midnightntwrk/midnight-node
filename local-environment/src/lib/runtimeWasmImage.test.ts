...
describe("selectRuntimeVariant", () => {
  it("prefers the compact compressed blob", () => {
    const dir = artifactsRoot();
    for (const f of ALL_VARIANTS) fs.writeFileSync(path.join(dir, f), "wasm");
    assert.equal(
  ...
  it("falls back through the variant order", () => {
    const dir = artifactsRoot();
    fs.writeFileSync(path.join(dir, "midnight_node_runtime.wasm"), "wasm");
    assert.equal(selectRuntimeVariant(dir), "midnight_node_runtime.wasm");
  });
...
describe("extractRuntimeWasmFromImage", () => {
  it("extracts the preferred variant into artifacts/from-image/<image>", () => {
    ...
    const result = extractRuntimeWasmFromImage({
      ...
      artifactsRoot: root,
      docker: fakeDocker({ layout: { amd64: ALL_VARIANTS } }),
    });
  ...
  it("re-extracts rather than reusing a stale blob from a moved tag", () => {
    const root = artifactsRoot();
    const docker = fakeDocker({ layout: { amd64: ALL_VARIANTS } });
