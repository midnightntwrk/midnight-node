...
/** Runtime wasm variants in submission preference order. */
export const RUNTIME_WASM_VARIANTS = [
  // What production upgrades submit, and the smallest extrinsic.
...
/** Pick the most preferred runtime variant present in `dir`. */
export function selectRuntimeVariant(dir: string): string | undefined {
  for (const variant of RUNTIME_WASM_VARIANTS) {
    if (fs.existsSync(path.join(dir, variant))) return variant;
  }
  return undefined;
}
 ...
 * in it. Re-extracts every call: image tags such as `latest` move, and a stale
 * blob would silently upgrade the wrong runtime.
 */
export function extractRuntimeWasmFromImage(
  ...
  try {
    for (const arch of architectures) {
      ...
      if (!variant) {
        failures.push(
          `/artifacts-${arch}: no runtime wasm among ${RUNTIME_WASM_VARIANTS.join(", ")}`,
        );
        continue;
      }
