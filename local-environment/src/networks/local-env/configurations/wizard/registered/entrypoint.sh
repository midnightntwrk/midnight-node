...
if [ -f "/shared/midnight-wizard-3.ready" ]; then
    echo "/shared/midnight-wizard-3.ready exists. Skipping configuration..."
    start_node
    exit 0
fi
...
spawn ./midnight-node wizards register1
set timeout 60
...
register2_command=$(echo "$register1_output" | sed -n '/\/midnight-node wizards register2 \\/,$p' | tr -d '\\\n')
...
expect "/midnight-node wizards register3"
expect eof
catch wait result
exit [lindex \$result 3]
EOF
...
register3_command=$(echo "$register2_output" | sed -n '/\/midnight-node wizards register3 \\/,$p' | tr -d '\\\n')
...
touch /shared/midnight-wizard-3.ready
