...
MIDNIGHT_NODE_IMAGE="ghcr.io/midnight-ntwrk/midnight-node:0.12.0-rc.3"
...
resource_limits_setup() {
  ...
  if [[ $restrict_resources == [Yy]* ]]; then
    ...
    if [[ $sensible_limits == [Yy]* ]]; then
      CPU_MIDNIGHT_NODE=0.4
      MEM_MIDNIGHT_NODE=400M
      cpu_cardano=1
      mem_cardano=1000M
      cpu_postgres=0.5
      mem_postgres=500M
      ...
      CPU_MIDNIGHT_NODE=$(validate_cpu_limit "Enter CPU limit for Midnight nodes (e.g., 0.4 for 0.4 CPU): ")
      MEM_MIDNIGHT_NODE=$(validate_memory_limit "Enter Memory limit for each of the 3 x Midnight nodes (e.g., 400M for 400 MB): ")
    ...
    CPU_MIDNIGHT_NODE=$DEFAULT_CPU_LIMIT
    MEM_MIDNIGHT_NODE=$DEFAULT_MEM_LIMIT
...
OGMIOS_PORT=1337
CPU_MIDNIGHT_NODE=0.000
MEM_MIDNIGHT_NODE=1000G
CPU_CARDANO=0.000
MEM_CARDANO=1000G
CPU_POSTGRES=0.000
MEM_POSTGRES=1000G
...
CPU_MIDNIGHT_NODE=$CPU_MIDNIGHT_NODE
MEM_MIDNIGHT_NODE=$MEM_MIDNIGHT_NODE
...
MIDNIGHT_NODE_IMAGE=${node_image:-$MIDNIGHT_NODE_IMAGE}
ARCHITECTURE=linux/$(uname -m)
EOF
...
choose_deployment_option() {
  ...
  if [[ $modify_stack =~ ^[Yy]$ ]]; then
    ...
    echo "4) Deploy a single Midnight node with network_mode: "host" for external connections (adjust midnight-external-node.txt before running this script)"
    echo "5) Deploy a 3 node Midnight Node network using wizard" 
...
create_docker_compose() {
    ...
    case $deployment_option in
      ...
      3)
        ...
        ;;
      4)
        echo -e "Including all services with external midnight node.\n"
        cat ./modules/cardano.txt >> docker-compose.yml
        cat ./modules/ogmios.txt >> docker-compose.yml
        cat ./modules/db-sync.txt >> docker-compose.yml
        cat ./modules/postgres.txt >> docker-compose.yml
        cat ./modules/midnight-external-node.txt >> docker-compose.yml
        cat ./modules/midnight-setup.txt >> docker-compose.yml
        ;;
      5)
        echo -e "Including all services with wizard midnight node.\n"
        cat ./modules/cardano.txt >> docker-compose.yml
        cat ./modules/ogmios.txt >> docker-compose.yml
        cat ./modules/db-sync.txt >> docker-compose.yml
        cat ./modules/postgres.txt >> docker-compose.yml
        cat ./modules/midnight-wizard.txt >> docker-compose.yml
        ;;
      0)
        ...
        cat ./modules/db-sync.txt >> docker-compose.yml
        cat ./modules/postgres.txt >> docker-compose.yml
        cat ./modules/midnight-nodes.txt >> docker-compose.yml
        cat ./modules/midnight-setup.txt >> docker-compose.yml
        cat ./modules/midnight-node-binary.txt >> docker-compose.yml
        ;;
      *)
        ...
        case "$1" in
            ...
            -d|--deployment-option)
                if [[ -n "$2" && "$2" =~ ^[1-4]$ ]]; then
                    ...
                    echo "Error: Invalid deployment option '$2'. Valid options are 1, 2, or 3, or 4."
                    exit 1
                fi
                ;;
            -p|--postgres-password)
            ...
            -h|--help)
                ...
                echo "  -d, --deployment-option   Specify one of the custom deployment options (1, 2, or 3, or 4)."
                ...
                echo "  -i, --node-image          Specify a custom Midnight Node image."
