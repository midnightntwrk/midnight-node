...
start_node() {
    ...
    /usr/local/bin/midnight-node \
        ...
        --node-key-file=/data/chains/midnight_template/network/secret_ed25519 \
        --base-path=/data \
        --keystore-path=/data/chains/midnight_template/keystore \
        --unsafe-rpc-external \
        --rpc-port=9933 \
        --rpc-cors=all \
...
if [ -f "/shared/midnight-wizard-1.ready" ]; then
    echo "/shared/midnight-wizard-1.ready exists. Skipping configuration and starting the node..."
    start_node
    exit 0
fi
...
expect <<EOF
spawn ./midnight-node wizards prepare-configuration
set timeout 180
expect "node base path (./data)"
send "\r"
...
while true; do
    if [ -f "/shared/midnight-wizard-2-keys.ready" ]; then
        break
    else
        sleep 1
    fi
...
governance_authority_public_keys=$(cat midnight-public-keys.json)
permissioned_candidate_public_keys=$(cat /shared/midnight-public-keys.json)
...
expect <<EOF
spawn ./midnight-node wizards create-chain-spec
expect "Do you want to continue? (Y/n)"
send "\r"
expect eof
EOF
...
expect <<EOF
spawn ./midnight-node wizards setup-main-chain-state
set timeout 300
...
touch /shared/midnight-wizard-1.ready
