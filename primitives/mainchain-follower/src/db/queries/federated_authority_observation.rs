...
/// Query to get the UTXO for a governance body by policy ID only
///
/// This query finds the most recent unspent UTXO that holds a native asset with the given policy ID.
/// Since governance NFTs use one-shot minting, there should only be one such UTxO.
///
/// This is simpler than `get_governance_body_utxo` as it doesn't require knowing the script address.
pub async fn get_utxo_by_policy_id(
	pool: &Pool<Postgres>,
	policy_id: &PolicyId,
	block_number: u32,
) -> Result<Option<GovernanceBodyUtxoRow>, SqlxError> {
