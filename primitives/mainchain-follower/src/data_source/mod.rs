...
use crate::db::{get_block_by_hash, get_datum_for_address, get_spent_datums_for_address};
use crate::{
...
impl MidnightNativeTokenObservationDataSource for MidnightNativeTokenObservationDataSourceImpl {
	async fn get_utxos_up_to_capacity(
		...
		let mut utxos = [
			...
			.await?,
			self.get_asset_spend_utxos(policy_id_hex, asset_name, start_position, end, capacity, 0)
				.await?,
		]
		.concat();
...
impl MidnightNativeTokenObservationDataSourceImpl {
	async fn get_registration_utxos(
		...
		let rows = get_datum_for_address(&self.pool, address, start, end, limit, offset)
			.await
		...
		for row in rows {
			...
			let Some(cardano_address) = list.get(0).as_bytes() else {
			...
			let Some(cardano_address) =
				String::from_utf8(cardano_address.clone()).ok().and_then(|bech32| {
				...
				log::error!(
					"Cardano address {cardano_address:?} not valid bech32 cardano address ({header:?})"
				);
				continue;
			};
	...
	async fn get_deregistration_utxos(
		...
		let rows = get_spent_datums_for_address(&self.pool, address, start, end, limit, offset)
			.await
			.map_err(|e| format!("Failed to fetch data: {e}"))?;
	...
	async fn get_asset_create_utxos(
		...
		let rows = crate::db::get_creates_for_asset_in_block_range(
			&self.pool,
			policy_id_hex,
			asset_name,
			start,
	...
	async fn get_asset_spend_utxos(
		...
		let rows = crate::db::get_spends_for_asset_in_block_range(
			&self.pool,
			policy_id_hex,
			asset_name,
			start,
