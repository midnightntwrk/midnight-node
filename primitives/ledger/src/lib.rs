...
/// Durable GC index blob (AuxStore): `block_hash → Anchored tip bytes`.
#[derive(
	Clone, Debug, Default, PartialEq, Eq, parity_scale_codec::Encode, parity_scale_codec::Decode,
)]
pub struct LedgerGcState {
	/// `(block_hash, tip_bytes)` — tip is `LedgerStateKey::Anchored` payload.
	pub tips: Vec<(Vec<u8>, Vec<u8>)>,
}

/// Durable backend for [`LedgerGcIndex`] (typically Substrate AuxStore).
pub trait LedgerGcDurability: Send + Sync {
	fn update(&self, f: &mut dyn FnMut(&mut LedgerGcState)) -> Result<(), String>;

	fn load(&self) -> Result<LedgerGcState, String>;
}

/// Node-side index of Anchored tips keyed by block hash (AuxStore).
///
/// # Flow
/// 1. Finality: read `Midnight::StateKey` at each finalized / stale hash →
///    record `(hash, tip)` in one AuxStore update.
/// 2. GC: once past pruning lag and `have_state_at` is false (debounced),
///    `unpersist` the tip once and drop the hash.
#[derive(Clone, Default)]
pub struct LedgerGcIndex {
	store: Option<Arc<dyn LedgerGcDurability>>,
}
...
impl LedgerGcIndex {
	...
	pub fn with_durability(store: Arc<dyn LedgerGcDurability>) -> Self {
		Self { store: Some(store) }
	}

	/// Finality: record many `(block_hash, tip_bytes)` in one AuxStore update.
	pub fn bind_finalized(&self, binds: &[(Vec<u8>, Vec<u8>)]) {
		if binds.is_empty() {
			return;
		}
		...
		if let Err(e) = store.update(&mut |state| {
			for (hash, tip) in binds {
				if hash.is_empty() || tip.is_empty() {
					continue;
				}
				if state.tips.iter().any(|(h, _)| h == hash) {
					continue;
				}
				state.tips.push((hash.clone(), tip.clone()));
			}
		}) {
			log::warn!(
				target: LOG_TARGET,
				"⚠️  Failed to durable-bind ledger GC tips ({e}); will retry on next finality"
			);
		}
	}

	/// Snapshot of bound tips for a GC slice (fresh load from AuxStore).
	pub fn bound_snapshot(&self) -> Vec<(Vec<u8>, Vec<u8>)> {
		match self.store.as_ref().map(|s| s.load()) {
			Some(Ok(state)) => state.tips,
			Some(Err(e)) => {
				log::warn!(target: LOG_TARGET, "⚠️  Failed to load ledger GC tips ({e})");
				Vec::new()
			},
			None => Vec::new(),
		}
	...
	/// Drop bound entries (one AuxStore update). Returns whether the removal
	/// was durably committed.
	///
	/// Callers performing a non-idempotent action per entry (the GC worker's
	/// blind `unpersist`) must remove **first** and act only on `true` —
	/// acting before a removal that then fails would repeat the action on the
	/// next slice (double-decrement, fail-deadly). A `true` removal followed
	/// by a failed action costs at most a leak.
	pub fn remove_bound(&self, block_hashes: &[Vec<u8>]) -> bool {
		...
		if let Err(e) = store.update(&mut |state| {
			state.tips.retain(|(h, _)| !block_hashes.iter().any(|x| x == h));
		}) {
			log::error!(
				target: LOG_TARGET,
				"💥 Failed to durable-remove reclaimed tips ({e}); deferred to next slice"
			);
			return false;
		}
		true
