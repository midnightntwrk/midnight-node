...
#[cfg(feature = "indexer-client")]
pub use crate::indexer_client::{IndexerClient, IndexerClientError};
...
impl<D: DB + Clone> LedgerContext<D> {
		...
		D: Sync,
	{
		use rayon::prelude::*;
		log::debug!(
			target: crate::ledger_9::transaction::PERF_TARGET,
	...
	pub fn latest_block_context(&self) -> BlockContext {
		self.latest_block_context
			...
			.unwrap_or_else(|| {
				...
				crate::ledger_9::make_block_context(now, Default::default(), Default::default())
			})
	}
