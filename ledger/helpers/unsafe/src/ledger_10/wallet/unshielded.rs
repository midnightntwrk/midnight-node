...
impl UnshieldedWallet {
	...
	// `pub(crate)` for the ledger-10 copy's `ecdsa_wallet_tests`, which feeds uniform bytes in
	// without HD.
	...
	// Only used by the ledger-10 copy's `ecdsa_wallet_tests`; unused on the pre-9 copies.
	#[cfg(test)]
	#[allow(dead_code)]
...
// tests live in the ledger-10 copy's `ecdsa_wallet_tests`: this file has one copy per generation,
...
mod tests {
	...
	use crate::ledger_10::WalletSeed;
