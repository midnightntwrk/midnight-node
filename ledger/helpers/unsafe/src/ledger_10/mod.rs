...
//! Ledger generation 10 toolkit apparatus: context/intent/offer/UTXO builders and
//! wallet key management, layered on top of the shared, version-independent
//! surface re-exported from `midnight-node-ledger-helpers::ledger_10`.
//!
//! Its `ledger_9/` counterpart is a separate copy on purpose: `diff -r ledger_9
//! ledger_10` shows exactly where the two generations diverge, and an edit here
//! cannot leak into v9.

pub use midnight_node_ledger_helpers::ledger_10::*;

pub use crate::extract_tx_with_context::extract_tx_with_context_ledger_10 as extract_tx_with_context;
...
/// Ledger 10 accepts either a legacy 2.x (`v6`) key, stored in the `V3` slot via the
/// crate-level (non-ledger-10-aliased) `transient_crypto` — the same 2.x
...
/// in (the entry point alone doesn't say which slot). Ledger 10 keys can land in either
...
/// loads as `ProvingKeyMaterial::ir_source`). Ledger 10+ stores these on-chain
