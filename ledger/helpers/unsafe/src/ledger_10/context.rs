...
impl<D: DB + Clone> LedgerContext<D> {
		...
		D: Sync,
	{
		use rayon::prelude::*;
		log::debug!(
			target: crate::ledger_10::transaction::PERF_TARGET,
	...
	pub fn latest_block_context(&self) -> BlockContext {
		self.latest_block_context
			...
			.unwrap_or_else(|| {
				...
				crate::ledger_10::make_block_context(now, Default::default(), Default::default())
			})
	}
	...
	fn compute_state_root(state: &LedgerState<D>) -> Option<Vec<u8>> {
		...
		crate::ledger_10::serialize(&sp.as_typed_key()).ok()
	}
