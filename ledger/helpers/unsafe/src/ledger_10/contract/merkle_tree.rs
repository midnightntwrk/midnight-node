...
lazy_static! {
	static ref RESOLVER: Resolver = crate::ledger_10::test_resolver("simple-merkle-tree");
}
...
impl<D: DB + Clone> Contract<D> for MerkleTreeContract {
	async fn deploy(
		...
		let store_op = crate::ledger_10::contract_operation_new(
			crate::ledger_10::verifier_key(self.resolver, "store").await,
			crate::ledger_10::ir_source(self.resolver, "store").await,
		)?;
		let check_op = crate::ledger_10::contract_operation_new(
			crate::ledger_10::verifier_key(self.resolver, "check").await,
			crate::ledger_10::ir_source(self.resolver, "check").await,
		)?;
	...
	fn contract_call(
		...
		ContractCallPrototype {
			...
			// Ledger 10 (transient-crypto 4) lets a call carry proofs it verifies inside its own
			// circuit; the test contracts have none.
