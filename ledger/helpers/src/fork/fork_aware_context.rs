...
impl ForkAwareLedgerContext {
	...
	///
	/// Seeds are passed in their ledger_9 form (the current `latest`). For
	/// older versions we re-deserialize from the raw bytes — the seed wire
	/// format is backwards-compatible across versions.
	...
	///
	/// Returns `None` if the context has already forked to a later version.
	...
	pub fn into_ledger9(self) -> Option<crate::ledger_9::context::LedgerContext<Db9>> {
		match self {
			...
			Self::Ledger7(_) | Self::Ledger8(_) => None,
		}
	}
}
