...
use crate::{
	BlockContext, DB, Deserializable, HashOutput, LedgerState, NetworkId, PedersenRandomness,
	ProofKind, Resolver, Serializable, SignatureKind, Timestamp, Transaction, TransactionContext,
	TransactionResult, Version, Versioned, Wallet, WalletSeed, deserialize, serialize,
	test_resolver,
};

lazy_static! {
	pub static ref DEFAULT_RESOLVER: Resolver = test_resolver("");
}
...
impl<D: DB + Clone> LedgerContext<D> {
		...
		for (block_context, txs) in groups_by_block_context {
			txs.iter().for_each(|tx| {
                ...
                if let TransactionResult::Failure(failure) = result {
                    println!("Failing result {:#?}\nof applying tx {:#?} \nto update Local Ledger State for tx_context {:#?}\n", failure, tx, tx_context)
                }
	...
	fn wallet_for_seed(
		...
		wallets.get_mut(&seed).unwrap_or_else(|| {
			panic!("Wallet with seed {:?} does not exists in the `LedgerContext", seed)
		})
	}
