...
use crate::{
	DB, KeyLocation, PUBLIC_PARAMS, PedersenRandomness, ProofMarker, ProofPreimageMarker, Resolver,
	ResolverTrait, Signature, StdRng, Transaction, ZswapResolver, test_resolver,
};
...
pub static RESOLVER: std::sync::LazyLock<Resolver> =
	std::sync::LazyLock::new(|| test_resolver("simple-merkle-tree"));
...
impl<D: DB + Clone> ProofProvider<D> for LocalProofServer {
	async fn prove(
		...
		tx.prove(rng, &*PUBLIC_PARAMS, resolver).await.expect("Tx should be provable")
	}
}
