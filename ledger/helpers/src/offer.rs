...
use crate::{
	...
	ProofPreimage, ShieldedTokenType, StdRng, Transient, VecStorage,
};
...
pub struct OfferInfo<D: DB + Clone> {
	pub inputs: Vec<Box<dyn BuildInput<D> + Send>>,
	pub outputs: Vec<Box<dyn BuildOutput<D> + Send>>,
	pub transients: Vec<Box<dyn BuildTransient<D> + Send>>,
}
...
impl<D: DB + Clone> OfferInfo<D> {
	pub fn build(
		...
		let inputs = VecStorage::from_std_vec(inputs);
		let outputs = VecStorage::from_std_vec(outputs);
		let transient = VecStorage::from_std_vec(transient);
		let deltas = VecStorage::from_std_vec(Self::calculate_offer_deltas(
			&inputs_balance,
			&outputs_balance,
		));
