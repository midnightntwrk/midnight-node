...
use crate::{
	ClaimMintTransaction, DB, HashMapStorage, Intent, IntentInfo, LedgerContext, Nonce, Offer,
	OfferInfo, Pedersen, PedersenRandomness, ProofMarker, ProofPreimage, ProofPreimageMarker,
	ProofProvider, SeedableRng, Signature, StdRng, Transaction, UnshieldedTokenType, WalletSeed,
};
...
pub struct StandardTrasactionInfo<D: DB + Clone> {
	...
	pub intents: HashMap<u16, IntentInfo<D>>,
...
impl<D: DB + Clone> StandardTrasactionInfo<D> {
	...
	pub fn set_intents(&mut self, intents: HashMap<u16, IntentInfo<D>>) {
