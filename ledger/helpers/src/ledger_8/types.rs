...
pub(crate) fn default_block_context() -> BlockContext {
	...
	crate::ledger_8::make_block_context(timestamp, HashOutput(parent_hash_bytes), timestamp)
}
	...
	Transaction<S, P, PureGeneratorPedersen, D>: Tagged,
{
	...
	pub fn serialize_inner(&self) -> Result<Vec<u8>, std::io::Error> {
		match &self {
			Self::Midnight(tx) => crate::ledger_8::serialize(tx),
			Self::System(tx) => crate::ledger_8::serialize(tx),
		}
	}
