...
use crate::{
	...
	Transaction, TransactionWithContext, Utxo, WalletSeed,
};
...
impl<D: DB + Clone> TryFrom<&WalletAddress> for Wallet<D> {
	type Error = Box<dyn std::error::Error>;

	fn try_from(value: &WalletAddress) -> Result<Self, Self::Error> {
		let shielded = ShieldedWallet::try_from(value)
			.map_err(|err| format!("could not create wallet from address: {err:?}"))?;
		let unshielded = UnshieldedWallet::try_from(value)
			.map_err(|err| format!("could not create wallet from address: {err:?}"))?;

		Ok(Self { root_seed: None, unshielded, shielded })
	}
}
...
impl<D: DB + Clone> Wallet<D> {
	pub fn default(root_seed: WalletSeed) -> Self {
		...
		Self { root_seed: Some(root_seed), shielded, unshielded }
	}
