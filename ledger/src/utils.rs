...
pub fn find_crate_version(crate_name: &str) -> Option<Vec<u8>> {
	...
	if let Ok(data) = cargo_toml {
		if let Some(value) = data.workspace.dependencies.get(crate_name) {
			match value {
				DependencyValue::String(version) => return Some(version.to_owned().into_bytes()),
				DependencyValue::Object { version: Some(version), .. } => {
					return Some(version.to_owned().into_bytes());
				},
				DependencyValue::Object { tag: Some(tag), .. } => {
					return Some(tag.to_owned().into_bytes());
				},
				DependencyValue::Object { rev: Some(rev), .. } => {
					return Some(rev.to_owned().into_bytes());
				},
				DependencyValue::Object { branch: Some(branch), .. } => {
					return Some(branch.to_owned().into_bytes());
				},
				_ => return None,
			};
		}
