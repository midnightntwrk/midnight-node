...
#[cfg(feature = "std")]
type Signature = crate::ledger_10::TransactionSignature;
...
#[cfg(feature = "std")]
type DbSeparate = crate::ledger_10::ledger_storage_local::db::ParityDb;
#[cfg(feature = "std")]
type DbUnified = crate::ledger_10::ledger_storage_local::db::ParityDb<
	sha2::Sha256,
	crate::ledger_10::ledger_storage_local::db::paritydb::OwnedDb,
...
/// Translate an older generation's `LedgerApiError` into its ledger-10 counterpart.
///
/// They are distinct types declared identically in `ledger_8/types.rs`, `ledger_9/types.rs`
/// and `ledger_10/types.rs`, so their SCALE encodings are identical by construction.
/// Round-tripping keeps this correct when a variant is added, where a hand-written match
/// would need editing in lockstep. `ledger_8_error_encoding_matches_ledger_10` and
/// `ledger_9_error_encoding_matches_ledger_10` guard the assumption.
#[cfg(all(feature = "std", feature = "legacy-ledgers"))]
fn as_ledger_10_error<E: parity_scale_codec::Encode>(error: E) -> LedgerApiError {
...
/// Falls through to the ledger-10 body otherwise.
///
...
macro_rules! serve_pre_migration_v8_read {
	($ext:expr, $state_key:expr, $call:ident($($arg:expr),* $(,)?)) => {
		if crate::is_ledger_8_state_key($state_key) {
			...
			return result.map_err(as_ledger_10_error);
		}
	};
}

/// Without `legacy-ledgers` there is no ledger-8 bridge to serve from: a ledger-8 root
/// falls through to the ledger-10 body, which reports it as undecodable.
#[cfg(all(feature = "std", not(feature = "legacy-ledgers")))]
macro_rules! serve_pre_migration_v8_read {
	($ext:expr, $state_key:expr, $call:ident($($arg:expr),* $(,)?)) => {};
}

#[runtime_interface]
pub trait Ledger10Bridge {
	...
	fn ensure_storage_initialized(&mut self) -> bool {
		use ledger_storage_ledger_10::storage::try_get_default_storage;
	...
	/// Translate the ledger state from ledger-v8 format to ledger-v9 format - which is also
	/// the ledger-10 format (`ledger-state[v18]`), so there is no separate 9->10 step.
	///
	/// Called by `pallet_midnight`'s v8->v9 storage migration during the runtime
	/// upgrade that crosses into ledger-10 straight from ledger 8. `state_key` is the
	/// pallet's `StateKey` (a v8 arena root); returns the new v9/v10 arena root to store
	/// back, together with the synthetic cost (picoseconds) the translation consumed
	/// against the ledger's cost model, for the pallet to charge as this migration's
	/// weight.
	fn migrate_state_v8_to_v9(
		...
		#[cfg(feature = "legacy-ledgers")]
		{
			...
			.map_err(as_ledger_10_error)
		}
		#[cfg(not(feature = "legacy-ledgers"))]
		{
			let _ = state_key;
			log::error!(
				target: crate::ledger_10::LOG_TARGET,
				"ledger 8 -> 9 state translation requested but ledger 8 is not compiled into this \
				 node (`legacy-ledgers` off)"
			);
	...
	fn register_benchmark_ledger_storage(&mut self) {
		...
		INIT.get_or_init(|| {
			...
			let _state_key = crate::ledger_10::storage::init_storage_paritydb_separate(
				dir,
...
#[cfg(all(test, feature = "std", feature = "legacy-ledgers"))]
mod tests {
	use super::as_ledger_10_error;
	use crate::{ledger_8::types as v8, ledger_9::types as v9, ledger_10::types as v10};

	/// `as_ledger_10_error` relies on the versions' `LedgerApiError` sharing a
	/// SCALE encoding, which holds because `ledger_8/types.rs`, `ledger_9/types.rs` and
	/// `ledger_10/types.rs` declare it identically. Pin that down — including a nested payload
	/// and the last variant, which is where a divergence would first show up — so a
	...
	#[test]
	fn ledger_8_error_encoding_matches_ledger_10() {
		let cases = [
			(v8::LedgerApiError::NoLedgerState, v10::LedgerApiError::NoLedgerState),
			(v8::LedgerApiError::ContractNotPresent, v10::LedgerApiError::ContractNotPresent),
			(v8::LedgerApiError::BeneficiaryNotFound, v10::LedgerApiError::BeneficiaryNotFound),
			(
				v8::LedgerApiError::Deserialization(v8::DeserializationError::TypedArenaKey),
				v10::LedgerApiError::Deserialization(v10::DeserializationError::TypedArenaKey),
			),
			(
				v8::LedgerApiError::Serialization(v8::SerializationError::LedgerParameters),
				v10::LedgerApiError::Serialization(v10::SerializationError::LedgerParameters),
			),
		];

		for (from, expected) in cases {
			assert_eq!(as_ledger_10_error(from.clone()), expected, "mistranslated {from:?}");
		}
	}

	#[test]
	fn ledger_9_error_encoding_matches_ledger_10() {
		let cases = [
			(v9::LedgerApiError::NoLedgerState, v10::LedgerApiError::NoLedgerState),
			(v9::LedgerApiError::ContractNotPresent, v10::LedgerApiError::ContractNotPresent),
			(v9::LedgerApiError::BeneficiaryNotFound, v10::LedgerApiError::BeneficiaryNotFound),
			(
