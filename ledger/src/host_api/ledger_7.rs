...
use crate::{
	common::types::{
		...
		TransactionAppliedStateRootWithEvents, TransactionDetails, Tx,
	},
...
pub trait LedgerBridge {
	 ...
	 * apply_transaction()
	 *
	 * The unversioned and `#[version(2)]` functions return the events-free
	 * `TransactionAppliedStateRoot`, byte-identical to a pre-ledger-events binary;
	 * `#[version(3)]` carries the events. WASM always calls the latest host-function
	 * version its imports declare, so this split guards the reverse direction: an old
	 * (events-free) runtime on a new binary decodes the events-free shape instead of
	 * silently truncating the events-carrying one. (A new runtime on an old binary
	 * fails at instantiation on the missing `#[version(3)]` import, not via a silent
	 * decode.)
	 */
