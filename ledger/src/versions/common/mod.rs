...
pub mod types;
use types::{DeserializationError, LedgerApiError};
...
use {
	api::{
		ContractAddress, ContractState, Ledger, LedgerParameters, Transaction,
		TransactionAppliedStage, TransactionOperation,
	},
	coin_structure_local::coin::Commitment,
	ledger_storage_local::{
		arena::{ArenaKey, Sp, VersionedArenaKey},
		db::DB,
		storage::{Map, default_storage},
	},
	midnight_primitives_ledger::LedgerMetricsExt,
	midnight_serialize_local::{Deserializable, NetworkId},
	mn_ledger_local::{
		semantics::TransactionContext,
		structure::{
			ContractAction, ContractCall, MaintenanceUpdate, ProofMarker, SignatureKind,
			SingleUpdate, Transaction as LedgerTransaction,
		},
	},
...
use crate::common::types::{
	...
	Hash, Op, StorageCost, TransactionAppliedStateRoot, TransactionDetails,
...
#[cfg(feature = "std")]
#[inline]
fn ensure_network_id(mut network_id: &[u8]) -> Result<NetworkId, LedgerApiError> {
	NetworkId::deserialize(&mut network_id, 0).map_err(|e| {
		log::error!(target: LOG_TARGET, "Error deserializing: Network Id: {:?}", e);
		LedgerApiError::Deserialization(DeserializationError::NetworkId)
	})
}
...
use {
	...
	mn_ledger_local::structure::{ClaimMintTransaction, LedgerState},
};
...
impl<S: SignatureKind<D> + std::fmt::Debug, D: DB> Bridge<S, D> {
	pub fn pre_fetch_storage(
		...
		let network_id = ensure_network_id(network_id)?;
		let api = api::new(network_id);
		let versioned_key: VersionedArenaKey<D::Hasher> = api.deserialize(state_key)?;
		let key: ArenaKey<D::Hasher> = versioned_key.into();
	...
	pub fn post_block_update(
		...
		let ledger = Ledger::post_block_update(ledger, block_context).map_err(|e| {
			log::error!(
				target: LOG_TARGET,
				"Post Block Update error: {:?}",
				e
			);
			LedgerApiError::NoLedgerState
		})?;

		let state_root = api.serialize(&ledger.hash())?;
	...
	pub fn apply_transaction(
		...
		let network_id = ensure_network_id(network_id)?;
		let api = api::new(network_id);
		let tx = api.deserialize::<Transaction<S, D>>(tx_serialized)?;
		log::info!(
			target: LOG_TARGET,
			"⚙️  Processing Tx {:?}",
			tx,
		...
		let mut event = TransactionAppliedStateRoot {
			state_root: api.serialize(&ledger.hash())?,
			tx_hash,
			all_applied,
			call_addresses: vec![],
			...
			claim_mints: vec![],
			unshielded_utxos_created: utxos.outputs(),
			unshielded_utxos_spent: utxos.inputs(),
		};

		for op in tx.calls_and_deploys() {
			match op {
				TransactionOperation::Call { address, .. } => {
					event.call_addresses.push(api.serialize(&address)?);
				},
				TransactionOperation::Deploy { address } => {
					event.deploy_addresses.push(api.serialize(&address)?);
				},
				TransactionOperation::Maintain { address } => {
					event.maintain_addresses.push(api.serialize(&address)?);
				},
				TransactionOperation::ClaimMint { value, coin_type } => {
					event.claim_mints.push((value, coin_type));
				},
			}
		}
	...
	) -> Result<Tx, LedgerApiError> {
		let network_id = ensure_network_id(network_id)?;
		let api = api::new(network_id);
		let tx = api.deserialize::<Transaction<S, D>>(transaction_bytes)?;
		let hash = tx.hash();
		let operations = tx.calls_and_deploys().try_fold(Vec::new(), |mut acc, cd| {
			let a = match cd {
				TransactionOperation::Call { address, entry_point } => {
					Op::Call { address: api.serialize(&address)?, entry_point }
				},
				TransactionOperation::Deploy { address } => {
					Op::Deploy { address: api.serialize(&address)? }
				},
				TransactionOperation::Maintain { address } => {
					Op::Maintain { address: api.serialize(&address)? }
				},
				TransactionOperation::ClaimMint { value, coin_type } => {
					Op::ClaimMint { coin_type, value }
				},
			};
			acc.push(a);
	...
	pub fn get_contract_state_json(
		...
		let f = json::contract_state_json;
	...
	pub fn get_contract_state(
		...
		let f = |contract_state| api.serialize(&contract_state);
	...
	pub fn get_zswap_chain_state(
		...
		api.serialize(&ledger.get_zswap_state(Some(addr)))
	}
	...
	pub fn mint_coins(
		...
		let sys_tx = api::SystemTransaction::Mint(vec![api::OutputInstructionUnshielded {
			amount,
			target_address,
			nonce: rng.r#gen(),
		}]);
		let ledger = Self::get_ledger(&api, state_key)?;
		let ledger = Ledger::apply_system_tx(ledger, &sys_tx)?;

		// Only update state after no errors
		ledger.persist();
		api.serialize(&ledger.hash())
	}
	...
	pub fn get_ledger_parameters(
		...
		api.serialize(&ledger_parameters)
	}

	// TODO: Needs to be redone with the new ledger cost model
	...
	fn get_ledger(api: &api::Api, state_key: &[u8]) -> Result<Sp<Ledger<D>, D>, LedgerApiError> {
		let key: VersionedArenaKey<D::Hasher> = api.deserialize(state_key)?;
	...
	fn get_transaction_details(
		...
		match ledger_tx {
			...
			LedgerTransaction::ClaimMint(_) => Ok(TransactionDetails::ClaimMint),
		}
	}
	...
	fn get_tx_type(tx: &Transaction<S, D>) -> &'static str {
		match tx.0 {
			...
			mn_ledger_local::structure::Transaction::ClaimMint(_) => "claim_mint",
		}
	}

	#[cfg(feature = "tx-validation-cache")]
	fn do_validate_transaction(
		...
		match TX_VALIDATION_CACHE.get(&tx_hash.0) {
			// Tx is cached: map `Ok` to `TransactionValidationWasCached::Yes`
			Some(validation) => validation.map(|_| TransactionValidationWasCached::Yes),
	...
	#[cfg(not(feature = "tx-validation-cache"))]
	fn do_validate_transaction(
		ledger: &Ledger,
		tx: &Transaction,
		_tx_hash: &WrappedHash,
		_tx_details: &TransactionDetails,
	) -> Result<TransactionValidationWasCached, LedgerApiError> {
		ledger.validate_transaction(tx).map(|_| TransactionValidationWasCached::No)
	}
	...
	pub fn build_claim_mint_transactions(
		...
		let unproven_tx = Transaction::ClaimMint(ClaimMintTransaction::from(
							...
							ContractAction::Call(call) => {
								if let Some(cstate) = new_state.index(call.address) {
									...
									if let Some(transcript) = transcript {
										let qcontext2 =
                                            qcontext.run_transcript(transcript, &mn_ledger_local::onchain_runtime::cost_model::DUMMY_COST_MODEL).unwrap(); //TODO: use real cost model
