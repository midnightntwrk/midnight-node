...
impl<D: DB> From<TransactionInvalid<D>> for InvalidError {
	fn from(error: TransactionInvalid<D>) -> Self {
		...
		match error {
			...
			// (`GenerationInfoAlreadyPresent` in 8 and again from 10, `InitialNonceAlreadyPresent`
			// in 9.1.0.0-rc.4+), so it is mapped in each `error_ext::ledger_N` instead of here.
