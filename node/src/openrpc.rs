...
// ---------------------------------------------------------------------------
// Declarative method registry (for drift detection tests)
// ---------------------------------------------------------------------------

/// Declarative registry of custom RPC method signatures.
/// The syn-based drift detection test compares trait source files against this
/// registry. If a method signature changes in the trait but not here, CI fails.
struct MethodSignature {
	/// JSON-RPC method name (e.g., "midnight_contractState")
	name: &'static str,
	/// Parameter names, excluding `&self`
	param_names: &'static [&'static str],
	/// Number of parameters (excluding &self)
	param_count: usize,
	/// Whether this is a partner-chains method (not locally parseable)
	is_partner_chains: bool,
}
...
const METHOD_REGISTRY: &[MethodSignature] = &[
	...
	MethodSignature {
		...
		param_names: &["contract_address", "at"],
		param_count: 2,
		is_partner_chains: false,
	},
	MethodSignature {
	...
	MethodSignature {
		name: "systemParameters_getTermsAndConditions",
		param_names: &["at"],
		param_count: 1,
		is_partner_chains: false,
	},
	MethodSignature {
		name: "systemParameters_getDParameter",
		param_names: &["at"],
		param_count: 1,
		is_partner_chains: false,
	},
	MethodSignature {
		name: "systemParameters_getAriadneParameters",
		param_names: &["epoch_number", "d_parameter_at"],
		param_count: 2,
		is_partner_chains: false,
	},
	// PeerInfoApi
	MethodSignature {
		name: "network_peerReputations",
		param_names: &[],
		param_count: 0,
		is_partner_chains: false,
	},
	MethodSignature {
		name: "network_peerReputation",
		param_names: &["peer_id"],
		param_count: 1,
		is_partner_chains: false,
	},
	MethodSignature {
		name: "network_unbanPeer",
		param_names: &["peer_id"],
		param_count: 1,
		is_partner_chains: false,
	},
	// SidechainRpcApi (partner-chains — not locally parseable)
	MethodSignature {
		name: "sidechain_getParams",
		param_names: &[],
		param_count: 0,
		is_partner_chains: true,
	},
	MethodSignature {
	...
	// SessionValidatorManagementRpcApi (partner-chains)
	MethodSignature {
		name: "sidechain_getEpochCommittee",
		param_names: &["epoch_number"],
		param_count: 1,
		is_partner_chains: true,
	},
	MethodSignature {
		name: "sidechain_getRegistrations",
		param_names: &["mc_epoch_number", "mc_public_key"],
		param_count: 2,
		is_partner_chains: true,
	},
	MethodSignature {
...
/// Source files containing #[rpc] trait definitions that can be parsed locally.
/// The grep discovery test verifies this list is complete.
#[cfg(test)]
const KNOWN_RPC_TRAIT_FILES: &[&str] = &[
	"pallets/midnight/rpc/src/lib.rs",
	"pallets/system-parameters/rpc/src/lib.rs",
	"node/src/peer_info_rpc.rs",
];
...
mod tests {
	...
	/// Parses local #[rpc] trait source files with syn, extracts method
	/// signatures, and compares against METHOD_REGISTRY. Catches parameter
	/// additions/removals, renames, and method signature changes.
	#[test]
	fn trait_signatures_match_registry() {
		let repo_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap();

		for file_path in KNOWN_RPC_TRAIT_FILES {
			let full_path = repo_root.join(file_path);
			let source = std::fs::read_to_string(&full_path)
				.unwrap_or_else(|e| panic!("Cannot read {}: {}", file_path, e));
			let syntax = syn::parse_file(&source)
				.unwrap_or_else(|e| panic!("Cannot parse {}: {}", file_path, e));
			...
			for item in &syntax.items {
				...
				let rpc_attr = trait_item.attrs.iter().find(|a| a.path().is_ident("rpc"));
				if rpc_attr.is_none() {
					continue;
				}

				let namespace = rpc_attr.and_then(|attr| {
					let mut ns = None;
				...
				for trait_method in &trait_item.items {
					let method = match trait_method {
						syn::TraitItem::Fn(m) => m,
						_ => continue,
					};

					let method_attr = method.attrs.iter().find(|a| a.path().is_ident("method"));
					let rpc_method_name = match method_attr {
						Some(attr) => {
							let mut name = None;
							let _ = attr.parse_nested_meta(|meta| {
					...
					let full_name = match &namespace {
						Some(ns) => format!("{}_{}", ns, rpc_method_name),
						None => rpc_method_name.clone(),
					};

					let registry_entry = METHOD_REGISTRY.iter().find(|r| r.name == full_name);
					let entry = match registry_entry {
						Some(e) => {
							assert!(
								!e.is_partner_chains,
								"Method '{}' in {} is marked as partner-chains in registry but found in local source",
								full_name, file_path
							);
							e
						},
						None => panic!(
							"Method '{}' found in {} but not in METHOD_REGISTRY. \
							 Add it to the registry and create OpenRPC metadata.",
							full_name, file_path
						),
					};
					...
					assert_eq!(
						params.len(),
						entry.param_count,
						"Method '{}' in {} has {} params but registry expects {}. \
						 Params in source: {:?}, in registry: {:?}",
						full_name,
						file_path,
						params.len(),
						entry.param_count,
					...
					assert_eq!(
						...
						entry.param_names,
						"Method '{}' in {} has params {:?} but registry expects {:?}. \
						 A parameter was renamed or reordered.",
						full_name,
						file_path,
						param_strs,
						entry.param_names
	...
	/// Scans the repository for #[rpc(] trait definitions and verifies all
	/// source files are listed in KNOWN_RPC_TRAIT_FILES. Catches new trait
	/// files added without updating the drift detection.
	#[test]
	fn all_rpc_trait_files_known() {
		let repo_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap();
			...
			.filter(|e| !e.path().components().any(|c| c.as_os_str() == "target"))
		{
			...
			if content.lines().any(|line| {
				...
				let relative = entry.path().strip_prefix(repo_root).unwrap();
				let rel_str = relative.to_string_lossy().to_string();
				if !rel_str.contains(".cargo") {
					found_files.push(rel_str);
				}
			}
		}
		found_files.sort();

		let mut known: Vec<String> = KNOWN_RPC_TRAIT_FILES.iter().map(|s| s.to_string()).collect();
		known.sort();

		let unknown: Vec<&String> = found_files.iter().filter(|f| !known.contains(f)).collect();
		assert!(
			unknown.is_empty(),
			"Found #[rpc(] trait definitions in files not listed in KNOWN_RPC_TRAIT_FILES:\n  {}\n\n\
			 Add these files to KNOWN_RPC_TRAIT_FILES so the signature drift test covers them.",
