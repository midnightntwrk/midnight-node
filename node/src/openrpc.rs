...
fn build_component_schemas() -> Value {
	...
	json!({
		...
		"RpcStateQuery": {
			"type": "object",
			"description": "A query into a contract's state tree. Each `path` element is a 0x-prefixed hex-encoded serialized AlignedValue interpreted as array index, map key, or merkle tree position depending on the StateValue variant at each level.",
			"properties": {
				"path": {
					"type": "array",
					"items": {
						...
						"description": "0x-prefixed hex-encoded serialized AlignedValue"
					}
				}
			},
			"required": ["path"]
