...
type MidnightService = sc_service::PartialComponents<
	...
	(
		...
		// recovered). Created in `new_partial` so it can wrap both import queues' block imports.
...
pub fn new_partial(
	...
	// Warp ledger-sync recovery gate, shared by both import queues (below), the authoring oracle, and
	// the recovery monitor (both in `new_full`). Wrapping each queue's block import here holds
	// post-warp block imports until the arena is recovered + verified. After the consensus flip,
	// `DispatchImportQueue` routes execute-bearing blocks to BABE, so the BABE sandwich must be
	// gated too — wrapping only AURA would let post-warp BABE blocks bypass arena recovery.
	...
	// Gate the *outer* sandwich (not innermost GRANDPA): `PartnerChainsBlockImport` runs inherent
	// checks that execute against the arena *before* calling inner, so wrapping only GRANDPA would
	// be too late. Same `RecoveryGate` as the AURA queue.
	let gated_babe_block_import = crate::warp_ledger_sync::block_import::GatedBlockImport::new(
		babe_block_import,
		recovery_gate.clone(),
		backend.clone(),
	);
	...
	let (babe_import_queue, babe_worker_handle) =
		sc_consensus_babe::import_queue(sc_consensus_babe::ImportQueueParams {
			...
			block_import: gated_babe_block_import,
...
pub async fn new_full<Network: sc_network::NetworkBackend<Block, <Block as BlockT>::Hash>>(
	...
	if role.is_authority() {
		...
		let authoring_sync_oracle = crate::warp_ledger_sync::oracle::MidnightSyncOracle::new(
			sync_service.clone(),
			warp_ledger_recovery_gate.clone(),
		);
		...
		let aura_worker = sc_consensus_aura::start_aura::<AuraPair, _, _, _, _, _, _, _, _, _, _>(
			StartAuraParams {
				...
				sync_oracle: authoring_sync_oracle.clone(),
		...
		let babe_worker = sc_consensus_babe::start_babe(sc_consensus_babe::BabeParams {
			...
			sync_oracle: authoring_sync_oracle,
			justification_sync_link: sync_service.clone(),
			create_inherent_data_providers: make_proposal_cidp(),
			force_authoring,
