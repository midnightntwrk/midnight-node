...
pub struct MidnightCfg {
	...
	pub main_chain_follower_mock_registrations_file: Option<String>,
...
fn main_chain_follower_vars(cfg: &MidnightCfg) -> Result<(), validation::Error> {
	let missing = |field: &str| {
		validation::Error::Custom(format!(
			"{} must be defined if ariadne is enabled (i.e. if use_main_chain_follower_mock is false)",
			field
		))
	};

	if cfg.use_main_chain_follower_mock {
		if cfg.main_chain_follower_mock_registrations_file.is_none() {
			return Err(validation::Error::Custom("main_chain_follower_mock_registrations_file must be defined if use_main_chain_follower_mock is true.".to_string()));
		}
	} else {
