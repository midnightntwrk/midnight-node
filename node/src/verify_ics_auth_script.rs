...
pub enum VerifyIcsAuthScriptError {
	#[error("I/O error: {0}")]
...
impl IcsVerificationResult {
	...
	pub fn print_summary(&self) {
		...
		Self::print_check("ICS Policy Hash", &self.policy_hash_check);
		Self::print_check("ICS Two-Stage Embedded", &self.two_stage_embedded_check);
...
fn load_ics_addresses(path: &Path) -> Result<IcsAddressesWithCode, VerifyIcsAuthScriptError> {
...
fn load_authorization_addresses(
	path: &Path,
) -> Result<AuthorizationAddresses, VerifyIcsAuthScriptError> {
	let content = std::fs::read_to_string(path)?;
...
async fn get_authorization_script_from_datum(
	...
	block_number: u32,
) -> Result<PolicyId, VerifyIcsAuthScriptError> {
	...
	.await?
	.ok_or_else(|| {
		VerifyIcsAuthScriptError::UtxoNotFound(format!(
...
fn decode_authorization_from_datum(datum: &DbDatum) -> Result<PolicyId, VerifyIcsAuthScriptError> {
	let list = datum.0.as_list().ok_or_else(|| {
		VerifyIcsAuthScriptError::DatumDecodeError("Expected datum to be a list".to_string())
	})?;

	if list.len() < 3 {
		return Err(VerifyIcsAuthScriptError::DatumDecodeError(format!(
	...
	let auth_bytes = auth_script_data.as_bytes().ok_or_else(|| {
		VerifyIcsAuthScriptError::DatumDecodeError(
...
async fn get_block_number(
...
) -> Result<u32, VerifyIcsAuthScriptError> {
	...
	let (block_number,) = row.ok_or_else(|| {
		VerifyIcsAuthScriptError::BlockNotFound(format!("Block {} not found", block_hash))
	})?;
...
fn verify_policy_hash(compiled_code_hex: &str, expected_policy_id: &PolicyId) -> CheckResult {
	match hex::decode(compiled_code_hex) {
		Ok(code_bytes) => {
			...
			if computed_hash == *expected_policy_id {
				CheckResult {
					passed: true,
					message: format!(
						"ICS policy_id {} matches blake2b_224(03 || compiled_code)",
				...
				CheckResult {
					passed: false,
					message: format!(
						"ICS policy hash mismatch: expected {}, computed {}",
						hex::encode(expected_policy_id.0),
		...
		Err(e) => CheckResult {
			passed: false,
			message: format!("ICS compiled_code hex decode error: {}", e),
		},
	}
}
		...
		Ok(code_bytes) => {
			if is_policy_id_embedded(&code_bytes, two_stage_policy_id) {
				CheckResult {
					passed: true,
					message: format!(
						"ICS two_stage_policy_id {} is embedded in compiled_code",
						hex::encode(two_stage_policy_id.0)
					),
				}
			} else {
				CheckResult {
					passed: false,
					message: format!(
						"ICS two_stage_policy_id {} NOT found in compiled_code",
						hex::encode(two_stage_policy_id.0)
					),
				}
			}
...
) -> Result<IcsVerificationResult, VerifyIcsAuthScriptError> {
