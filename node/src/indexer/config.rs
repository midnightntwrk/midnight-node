...
impl Default for SpoApplicationConfig {
	fn default() -> Self {
		Self { interval: spo_interval_default(), stake_refresh: spo_stake_refresh_default() }
	}
}
...
impl From<SpoApplicationConfig> for spo_app::Config {
	fn from(config: SpoApplicationConfig) -> Self {
		Self { interval: config.interval, stake_refresh: config.stake_refresh }
	}
}
...
fn spo_stake_refresh_default() -> StakeRefreshConfig {
	StakeRefreshConfig { period_secs: 900, page_size: 100, max_rps: 2 }
}
