...
use midnight_node_runtime::{Block, EXISTENTIAL_DEPOSIT};
	...
	match subcommand {
		...
		Subcommand::PartnerChains(cmd) => {
			...
			let make_dependencies = |config: sc_service::Configuration| {
				...
				Ok((client, task_manager, other.3.authority_selection))
			};

			partner_chains_node_commands::run(&cfg, make_dependencies, cmd.clone())
		},
