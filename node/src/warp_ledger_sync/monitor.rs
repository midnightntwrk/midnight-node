	...
	Network: NetworkRequest + Send + Sync + ?Sized + 'static,
{
	...
	let driver = LedgerSyncClient::new(client, network, protocol_name, unified);
	loop {
		let peers: Vec<_> = match sync_service.peers_info().await {
			Ok(info) => info.into_iter().map(|(peer, _)| peer).collect(),
			Err(_) => Vec::new(),
		};
