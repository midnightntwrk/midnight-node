...
///
/// Read by raw key because no runtime API exposes it, and unlike `pallet_midnight::StateKey` it
/// cannot be reached through the runtime's own type here: `pallet_cnight_observation` is only
/// transiently in the runtime for the ledger 8 -> 9 fork.
...
mod storage_key_tests {
	...
	/// `pre_fork_state_key_storage_key` is hand-built from pallet/item name strings, so a pallet
	/// rename is a silent break: because the dust-replay guard fails *open*, it would stop firing
	/// with no compile error. Pin it to the real runtime's derived key.
	///
	/// `StateKey` needs no such test any more — `read_state_key` derives that key from
	/// `pallet_midnight::StateKey::<Runtime>::hashed_key()` directly, so a rename is a compile
