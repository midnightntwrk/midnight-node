...
impl<T: Keystore> Keystore for AuraToBabeMigrationKeystore<T> {
	fn sr25519_public_keys(
		...
		if loaded.is_empty() && key_type == key_types::BABE {
			let aura = self.keystore.sr25519_public_keys(key_types::AURA);
			if !aura.is_empty() {
				log::warn!(
					target: LOG_TARGET,
	...
	fn sr25519_sign(
		...
		let signed = self.keystore.sr25519_sign(key_type, public, msg);
		match signed {
			Ok(None) if key_type == key_types::BABE => {
				self.keystore.sr25519_sign(key_types::AURA, public, msg)
			},
			_ => signed,
		}
	}
	...
	fn sr25519_vrf_sign(
		...
		let signed = self.keystore.sr25519_vrf_sign(key_type, public, data);
		match signed {
			Ok(None) if key_type == key_types::BABE => {
				self.keystore.sr25519_vrf_sign(key_types::AURA, public, data)
			},
			_ => signed,
		}
	}
	...
	fn sr25519_vrf_pre_output(
		...
		let output = self.keystore.sr25519_vrf_pre_output(key_type, public, input);
		match output {
			Ok(None) if key_type == key_types::BABE => {
				self.keystore.sr25519_vrf_pre_output(key_types::AURA, public, input)
			},
			_ => output,
		}
	}
	...
	fn keys(&self, key_type: sp_runtime::KeyTypeId) -> Result<Vec<Vec<u8>>, sp_keystore::Error> {
		...
		match keys {
			Ok(ks) if ks.is_empty() && key_type == key_types::BABE => {
				let aura = self.keystore.keys(key_types::AURA);
	...
	fn has_keys(&self, public_keys: &[(Vec<u8>, sp_runtime::KeyTypeId)]) -> bool {
		for (key, key_type) in public_keys {
			...
			if *key_type == key_types::BABE
				&& self.keystore.has_keys(&[(key.clone(), key_types::AURA)])
			{
				log::warn!(
					target: LOG_TARGET,
...
mod tests {
	...
	fn public_keys_returns_babe_keys_when_present() {
		...
		let babe = gen_key(&inner, key_types::BABE, "//Babe-seed");
		let _aura = gen_key(&inner, key_types::AURA, "//Aura-seed");

		let keys = keystore.sr25519_public_keys(key_types::BABE);
	...
	fn public_keys_falls_back_to_aura_when_babe_missing() {
		let (inner, keystore) = mk_keystore();
		let aura = gen_key(&inner, key_types::AURA, "//Aura-seed");

		assert_eq!(keystore.sr25519_public_keys(key_types::BABE), vec![aura]);
	}
	...
	fn public_keys_aura_request_is_passthrough() {
		let (inner, keystore) = mk_keystore();
		let aura = gen_key(&inner, key_types::AURA, "//Aura-seed");
		gen_key(&inner, key_types::BABE, "//Babe-seed");

		// An AURA request must return only AURA keys, never BABE.
		assert_eq!(keystore.sr25519_public_keys(key_types::AURA), vec![aura]);
	}

	#[test]
	fn public_keys_empty_when_neither_present() {
		let (_inner, w) = mk_keystore();
		assert!(w.sr25519_public_keys(key_types::BABE).is_empty());
	}
	...
	fn sign_babe_falls_back_to_aura_key() {
		...
		let sig = keystore.sr25519_sign(key_types::BABE, &aura, MSG).unwrap().unwrap();
	...
	fn sign_babe_uses_babe_key_when_present() {
		...
		let sig = keystore.sr25519_sign(key_types::BABE, &babe, MSG).unwrap().unwrap();
	...
	fn sign_with_aura_key_type_works() {
		...
		let sig = w.sr25519_sign(key_types::AURA, &aura, MSG).unwrap().unwrap();
		assert!(sr25519::Pair::verify(&sig, MSG, &aura));
	}
	...
	fn sign_babe_unknown_key_returns_none() {
		let (inner, w) = mk_keystore();
		gen_key(&inner, key_types::AURA, "//Aura-seed");
		let stranger = sr25519::Pair::from_string("//Stranger", None).unwrap().public();

		assert!(w.sr25519_sign(key_types::BABE, &stranger, MSG).unwrap().is_none());
	}
	...
	fn vrf_sign_falls_back_to_aura_key() {
		...
		let sig = w.sr25519_vrf_sign(key_types::BABE, &aura, &data).unwrap();
	...
	fn vrf_pre_output_falls_back_to_aura_key() {
		...
		let pre = w.sr25519_vrf_pre_output(key_types::BABE, &aura, &input).unwrap();
	...
	fn keys_falls_back_to_aura_when_babe_missing() {
		...
		assert_eq!(w.keys(key_types::BABE).unwrap(), vec![raw(&aura)]);
	}
	...
	fn keys_returns_babe_keys_when_present() {
		...
		assert_eq!(w.keys(key_types::BABE).unwrap(), vec![raw(&babe)]);
	}
	...
	fn has_keys_falls_back_to_aura_for_babe() {
		...
		assert!(w.has_keys(&[(raw(&aura), key_types::BABE)]));
		assert!(w.has_keys(&[(raw(&aura), key_types::AURA)]));
	}
	...
	fn has_keys_missing_babe_returns_false() {
		...
		assert!(!w.has_keys(&[(raw(&stranger), key_types::BABE)]));
	}
	...
	fn has_keys_mixed_missing_non_babe_returns_false() {
		...
		assert!(!w.has_keys(&[(raw(&aura), key_types::BABE), (vec![9u8; 32], GRANDPA)]));
	}
	...
	fn ed25519_babe_request_does_not_fall_back() {
		...
		assert!(w.ed25519_public_keys(key_types::BABE).is_empty());
	}
}
