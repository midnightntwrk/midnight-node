...
use pallet_midnight::pallet::{
	CallDetails, ClaimRewardsDetails, DeploymentDetails, MaintainDetails, PayoutDetails,
	TxAppliedDetails,
};
	...
	let translated: Vec<EventType> = events
		...
		.map(|event| match event {
			...
			EventRecord {
				phase,
				event:
					RuntimeEvent::Midnight(MidnightEvent::ClaimRewards(ClaimRewardsDetails {
						tx_hash: hash,
						value,
					})),
				topics,
			} => EventType::MidnightClaimRewards {
				phase: phase.into(),
				topics: topics.into(),
				value: *value,
				tx_hash: to_hex(hash),
