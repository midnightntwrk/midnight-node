...
pub use frame_support::{
	...
	traits::{
		...
		KeyOwnerProofSystem, Nothing, Randomness, StorageInfo,
	},
	weights::{
		IdentityFee, Weight,
		constants::{
...
use sp_runtime::{
	ApplyExtrinsicResult, MultiSignature, create_runtime_str, generic, impl_opaque_keys,
	traits::{
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("midnight"),
	impl_name: create_runtime_str!("midnight"),
	authoring_version: 1,
...
) -> Option<BoundedVec<(CrossChainPublic, SessionKeys), MaxAuthorities>> {
...
impl pallet_session_validator_management::Config for Runtime {
	...
	fn select_authorities(
		...
		sidechain_epoch: ScEpochNumber,
	) -> Option<BoundedVec<(Self::AuthorityId, Self::AuthorityKeys), MaxAuthorities>> {
...
impl_runtime_apis! {
	...
	impl pallet_midnight::MidnightRuntimeApi<Block> for Runtime {
		fn get_contract_state_json(contract_address: Vec<u8>) -> Result<Vec<u8>, LedgerApiError> {
			Midnight::get_contract_state_json(&contract_address)
		}
		fn get_contract_state(contract_address: Vec<u8>) -> Result<Vec<u8>, LedgerApiError> {
			Midnight::get_contract_state(&contract_address)
		}
		fn get_decoded_transactions(extrinsics: Vec<<Block as BlockT>::Extrinsic>) -> Vec<TransactionTypeV2>  {
			extrinsics.into_iter().map(|tx| {
				match tx.function {
					RuntimeCall::Midnight(MidnightCall::send_mn_transaction { midnight_tx }) => {
						TransactionTypeV2::MidnightTx(midnight_tx.clone(), Midnight::get_decoded_transaction(&midnight_tx))
					},
					RuntimeCall::Timestamp(TimestampCall::set { now } ) => TransactionTypeV2::TimestampTx(now),
					_ => TransactionTypeV2::UnknownTx
				}
			}).collect()
		}
	...
	> for Runtime {
		...
		fn calculate_committee(authority_selection_inputs: AuthoritySelectionInputs, sidechain_epoch: sidechain_domain::ScEpochNumber) -> Option<Vec<(CrossChainPublic, SessionKeys)>> {
	...
	impl midnight_primitives_native_token_management::NativeTokenObservationApi<Block> for Runtime {
		fn get_mapping_validator_address() -> Vec<u8> {
			pallet_native_token_observation::MainChainGenerationRegistrantsAddress::<Runtime>::get().into_inner()
		}
		fn get_next_block_range() -> (i64, i64) {
			NativeTokenObservation::get_next_block_range()
		}
...
mod tests {
	...
	use sidechain_domain::{
		...
		MainchainPublicKey, MainchainSignature, PermissionedCandidateData, RegistrationData,
		ScEpochNumber, SidechainSignature, StakeDelegation, UtxoId, UtxoInfo,
	};
		...
		let candidates: Vec<CandidateRegistrations> = validators
			...
			.map(|validator| {
				...
				let registration_data = RegistrationData {
					...
					mainchain_signature: MainchainSignature(mock_mainchain_signature.0.to_vec()),
