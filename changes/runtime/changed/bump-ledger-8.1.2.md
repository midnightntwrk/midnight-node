...
Moves the ledger 8 pin from 8.1.0 + the Array signature-verification fix to
8.1.2, which carries that fix plus:
...
- hardening of low-level deserialization across `serialize`, `base-crypto`,
  ...
  than decoded, so an 8.1.2 ledger rejects data an 8.1.0 ledger accepts
