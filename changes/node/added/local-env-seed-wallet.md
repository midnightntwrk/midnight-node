...
Adds a `local-env-seed` docker-compose job that funds the well-known dev wallet
...
- The funding **spends the seeded faucet cNIGHT** (the circulating pool minted by
  the `cnight-seeder` step) rather than minting fresh, so #1778's pools and the
...
- Implemented as a feature-gated e2e test `tests/e2e/tests/local_env_seed.rs`
  (`local-env-seed` feature, built alongside `local`) so it never joins the normal
...
- `earthly +local-env-seeder-image` compiles the test into a runnable image
  (`midnight-node-seeder`); the local-env runner builds it on first use. The job
  runs once after the chain is up (after `midnight-setup` + `cnight-seeder`) and is
  idempotent via a `runtime-values/wallets-seeded` marker.
