...
`governance` (submit the runtime wasm extracted from `new_node_image`
through the federated-authority governance flow via
`npm run governance-runtime-upgrade:${NETWORK}`), and `full` (image
rollout followed by the governance runtime upgrade via
`npm run full-upgrade:${NETWORK}`). An `allow_same_version` input maps
to the tooling's `--allow-same-version` rehearsal escape hatch.

Governance modes extract the candidate runtime wasm from the new node
image's `/artifacts-<arch>/` directory and, after the upgrade, assert
