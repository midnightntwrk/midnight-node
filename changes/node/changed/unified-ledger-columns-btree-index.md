...
**This changes the on-disk column layout of `unified` databases.** A `unified` database created before this release will refuse to start, because nodes already written outside a btree cannot be re-indexed in place — delete the chain data directory and resync. Databases written in `separate` mode are unaffected: that layout is unchanged, and it is the one deployed nodes are upgrading from — they open normally and are then migrated in place.
