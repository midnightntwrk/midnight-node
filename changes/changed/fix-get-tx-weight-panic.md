...
The fix replaces `.expect()` with `.unwrap_or(0)`, allowing the transaction to proceed to validation where `BlockLimitExceededError` is properly returned as an `InvalidTransaction` error.
