...
pub use midnight_primitives::{TransactionType, TransactionTypeV2};
...
pub mod pallet {
	...
	use midnight_node_ledger::types::{
		self as LedgerTypes, BenchmarkComponents, GasCost, StorageCost, TransactionDetails,
		Tx as LedgerTx, UtxoInfo, active_ledger_bridge as LedgerApi,
		active_version::{
	...
	// Gas cost for the simplest contract call (`simple-merkle-tree`).
	// The value is taken from the logs when running the `execute_contract_call` benchmark.
	// This value should be reviewed and updated in necessary for each new release.
	const BASE_GAS_COST: GasCost = 20_262;
	// Tx size used during the benchmarks to calculate `deserialize_transaction` times.
	const TX_SIZE_REFERENCE: u64 = 1381992;
	...
	// We specify the max despite this technically being a bounded vec. We do this so that we can avoid having to specify this pallet
	// as one that contains unbounded state because we are aware of the issue and have a plan to move away from this
	pub type MaxStateLength = ConstU32<{ u32::MAX }>;
	type MaxNetworkIdLength = ConstU32<1>;
	#[pallet::storage]
	#[pallet::getter(fn state_key)]
	// Learn more about declaring storage items:
	// https://docs.substrate.io/main-docs/build/runtime-storage/#declaring-storage-items
	//this is workaround for not integrating directly with MidnightLedger storage mechanism
	//which is still work in progress
	pub type StateKey<T> = StorageValue<_, BoundedVec<u8, MaxStateLength>>;
	...
	pub struct ClaimMintDetails {
		pub tx_hash: LedgerTypes::Hash,
		pub coin_type: LedgerTypes::Hash,
		pub value: u128,
	}
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		fn on_initialize(_block: BlockNumberFor<T>) -> Weight {
			...
			LedgerApi::pre_fetch_storage(&network_id, &state_key)
				.expect("Failed to pre-fetch storage");
		...
		fn on_finalize(_block: BlockNumberFor<T>) {
			...
			let state_root = LedgerApi::post_block_update(&network_id, &state_key, block_context)
			...
			if let Some(beneficiary) = beneficiary {
				...
				match LedgerApi::mint_coins(&network_id, &state_key, reward, &beneficiary[..]) {
					Ok(new_state_key) => {
						log::info!("Minting {:?} coins for {:?}", reward, beneficiary);
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight({
                if SafeMode::<T>::get() {
                    ...
                    match LedgerApi::validate_transaction(
                        &Pallet::<T>::get_network_id(),
                        &<StateKey<T>>::get().unwrap(),
                        midnight_tx,
                        Pallet::<T>::get_block_context(),
                        runtime_version,
                    ) {
                        Ok((_, tx_details)) => {
                            match BenchmarkComponents::try_from(&tx_details) {
                                Ok(components) => {
                                    let mut weight = <T as Config>::WeightInfo::send_standard_transaction(
                                        components.num_guaranteed_inputs,
                                        components.num_guaranteed_outputs,
                                        components.num_guaranteed_transients,
                                        components.num_fallible_inputs,
                                        components.num_fallible_outputs,
                                        components.num_fallible_transients,
                                        components.num_contracts_deploy,
                                        components.num_contract_replace_auth,
                                        components.num_contract_key_remove,
                                        components.num_contract_key_insert,
                                        components.num_contract_operations,
                                    );

                                    let calls_extra_weight = Pallet::<T>::calculate_extra_contract_calls_weight(
                                        tx_details.clone(),
                                        components.num_contract_operations as u64
                                    );

                                    weight = weight.saturating_add(calls_extra_weight);

                                    let extra_size_weight = Pallet::<T>::calculate_extra_size_weight(
                                        midnight_tx,
                                    );

                                    weight.saturating_add(extra_size_weight)
                                },
                                // Means it is a `Transaction::ClaimMint` tx
                                Err(_) => {
                                    <T as Config>::WeightInfo::send_claim_mint_transaction()
                                }
                            }
                        },
                        // Weight should return `0` instead of `u64::MAX`` in case the validation fails.
                        // Otherwise the user will receive a misleading Exhaust Block Limit erro when trying to submit the tx.
                        // Having weight `0`, the tx will be validated and rejected afterwards.
		...
		pub fn send_mn_transaction(_origin: OriginFor<T>, midnight_tx: Vec<u8>) -> DispatchResult {
			...
			for (value, coin_type) in result.claim_mints {
				let claim_event = Event::ClaimMint(ClaimMintDetails { tx_hash, coin_type, value });
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn get_contract_state_json(contract_address: &[u8]) -> Result<Vec<u8>, LedgerApiError> {
			let network_id = Self::get_network_id();
			let state_key = StateKey::<T>::get().expect("Failed to get state key");
			LedgerApi::get_contract_state_json(&network_id, &state_key, contract_address)
		}

		pub fn get_contract_state(contract_address: &[u8]) -> Result<Vec<u8>, LedgerApiError> {
			let network_id = Self::get_network_id();
			let state_key = StateKey::<T>::get().expect("Failed to get state key");
			LedgerApi::get_contract_state(&network_id, &state_key, contract_address)
		}
		...
		pub fn get_decoded_transaction(
			...
			let network_id = Self::get_network_id();
			LedgerApi::get_decoded_transaction(&network_id, midnight_transaction)
		}
		...
		pub fn get_zswap_chain_state(contract_address: &[u8]) -> Result<Vec<u8>, LedgerApiError> {
			let network_id = Self::get_network_id();
			let state_key = StateKey::<T>::get().expect("Failed to get state key");
			LedgerApi::get_zswap_chain_state(&network_id, &state_key, contract_address)
		}
		// grcov-excl-stop
		...
		pub fn get_unclaimed_amount(beneficiary: &[u8]) -> Result<u128, LedgerApiError> {
			let network_id = Self::get_network_id();
			let state_key = StateKey::<T>::get().expect("Failed to get state key");
			LedgerApi::get_unclaimed_amount(&network_id, &state_key, beneficiary)
		}

		pub fn get_ledger_parameters() -> Result<Vec<u8>, LedgerApiError> {
			let network_id = Self::get_network_id();
			let state_key = StateKey::<T>::get().expect("Failed to get state key");
			LedgerApi::get_ledger_parameters(&network_id, &state_key)
		}
		...
		pub fn get_transaction_cost(tx: &[u8]) -> Result<(StorageCost, GasCost), LedgerApiError> {
			...
			let state_key = StateKey::<T>::get().expect("Failed to get state key");
			let block_context = Self::get_block_context();
			LedgerApi::get_transaction_cost(&network_id, &state_key, tx, block_context)
		}

		pub fn get_zswap_state_root() -> Result<Vec<u8>, LedgerApiError> {
			let network_id = Self::get_network_id();
			let state_key = StateKey::<T>::get().expect("Failed to get state key");
			LedgerApi::get_zswap_state_root(&network_id, &state_key)
		}
		...
		fn calculate_extra_contract_calls_weight(
			tx_details: TransactionDetails,
			calls_num: u64,
		) -> Weight {
			let base_gas_cost = BASE_GAS_COST.saturating_mul(calls_num);
			let tx_gas_cost: GasCost = tx_details.into();
			let extra_gas_cost = tx_gas_cost.saturating_sub(base_gas_cost);

			// Calculate the weight for the extra gas cost using the rule of three
			let numerator = extra_gas_cost
				.saturating_mul(<T as Config>::WeightInfo::execute_contract_call().ref_time());
			let denominator = BASE_GAS_COST;
			let extra_gas_cost_weight =
				Weight::from_parts(numerator.saturating_div(denominator), 0);

			// Add extra weight per contract call in case benchmarks are not accurate enough
			let extra_weight = ConfigurableContractCallWeight::<T>::get().saturating_mul(calls_num);

			extra_gas_cost_weight.saturating_add(extra_weight)
		}

		fn calculate_extra_size_weight(tx: &[u8]) -> Weight {
			let tx_size = tx.len() as u64;

			// Calculate the tx size weight
			let numerator = tx_size
				.saturating_mul(<T as Config>::WeightInfo::deserialize_transaction().ref_time());
			let denominator = TX_SIZE_REFERENCE;
			let extra_tx_size_weight = Weight::from_parts(numerator.saturating_div(denominator), 0);

			// Add extra weight for size
			let extra_weight = ConfigurableTransactionSizeWeight::<T>::get();

			extra_tx_size_weight.saturating_add(extra_weight)
		}
	}
}
