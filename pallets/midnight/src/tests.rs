...
use midnight_node_ledger::types::{
	BlockContext,
	active_version::{DeserializationError, InvalidError, LedgerApiError, TransactionError},
};
...
fn test_get_contract_state() {
	mock::new_test_ext().execute_with(|| {
		...
		assert_ok!(mock::Midnight::send_mn_transaction(
			RuntimeOrigin::none(),
			hex::encode(tx_deploy).as_bytes().to_vec()
		));
		process_block(2, block_context_store.into());

		assert_ok!(mock::Midnight::send_mn_transaction(
			RuntimeOrigin::none(),
			hex::encode(tx_store).as_bytes().to_vec()
		));
		process_block(3, block_context_check.into());

		assert_ok!(mock::Midnight::send_mn_transaction(
			RuntimeOrigin::none(),
			hex::encode(tx_check).as_bytes().to_vec()
		));
		process_block(4, block_context_maintenance.into());

		assert_ok!(mock::Midnight::send_mn_transaction(
			RuntimeOrigin::none(),
			hex::encode(tx_maintenance).as_bytes().to_vec()
		));
...
fn test_validation_works() {
	...
	let call =
		MidnightCall::send_mn_transaction { midnight_tx: hex::encode(tx).as_bytes().to_vec() };
	mock::new_test_ext().execute_with(|| {
		init_ledger_state(block_context.into());
...
fn disable_safe_mode() {
	mock::new_test_ext().execute_with(|| {
		...
		let tx = hex::encode(tx).into_bytes();
...
fn test_get_mn_transaction_fee() {
	mock::new_test_ext().execute_with(|| {
		...
		let tx = hex::encode(tx).as_bytes().to_vec();
