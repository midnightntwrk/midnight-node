...
fn test_send_mn_transaction() {
	mock::new_test_ext().execute_with(|| {
		let (tx, block_context) =
			midnight_ledger_unsafe_helpers::latest::extract_tx_with_context(DEPLOY_TX);
...
fn test_send_mn_transaction_invalid_tx() {
	mock::new_test_ext().execute_with(|| {
		let (tx, block_context) =
			midnight_ledger_unsafe_helpers::latest::extract_tx_with_context(STORE_TX);
		init_ledger_state(block_context.into());
...
fn test_get_contract_state() {
	mock::new_test_ext().execute_with(|| {
		...
		let (tx_check, block_context_check) =
			midnight_ledger_unsafe_helpers::latest::extract_tx_with_context(CHECK_TX);
		let (tx_maintenance, block_context_maintenance) =
			midnight_ledger_unsafe_helpers::latest::extract_tx_with_context(MAINTENANCE_TX);
...
fn test_send_zswap_tx() {
	mock::new_test_ext().execute_with(|| {
		let (tx, block_context) =
			midnight_ledger_unsafe_helpers::latest::extract_tx_with_context(ZSWAP_TX);
