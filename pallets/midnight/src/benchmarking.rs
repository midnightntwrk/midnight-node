...
mod benchmarks {
	...
	fn send_standard_transaction(
		...
		// Send setup_tx
		assert_ok!(Midnight::<T>::send_mn_transaction(
			RawOrigin::None.into(),
			hex::encode(setup_tx).into_bytes()
		));

		// CODE TO BENCHMARK
		#[extrinsic_call]
		send_mn_transaction(RawOrigin::None, hex::encode(tx.clone()).into_bytes());
	...
	fn send_claim_mint_transaction() {
		...
		assert!(tx_details == TransactionDetails::ClaimMint);
	}
	...
	fn execute_contract_call() {
		...
		#[block]
		{
			LedgerApi::execute_contract_call(
				&network_id,
				&ledger_state_key,
				&hex::encode(tx.clone()).into_bytes(),
			)
			.unwrap();
		}
	...
	fn deserialize_transaction() {
		...
		let tx_bytes = hex::encode(tx.clone()).into_bytes();
