...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		fn migrate_to_babe(slot: Slot) {
			...
			// `CurrentSlot` must be *this* block's slot, not the genesis slot: this flip block is the
			// last pre-BABE block (slot `slot`), and pallet-babe's `OnTimestampSet` asserts
			// `CurrentSlot == timestamp_slot` later in the same block. It momentarily sits one slot
			// below `GenesisSlot`, which is fine — the first BABE block (`slot + 1`) advances it to
			// `GenesisSlot`. (pallet-babe's `initialize` also derives the same value from this block's
			// BABE pre-digest; setting it here is order-independent.)
