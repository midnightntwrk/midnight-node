...
if [[ -z "$DBSYNC_POD_NAME" ]]; then
    ...
    DBSYNC_POD_NAME="$(kubectl get pods -n qanet -o name --context "$context_name" | grep 'db-sync-cardano-01-0')"
...
if [[ -z "$DBSYNC_POSTGRES_POD_NAME" ]]; then
    ...
    DBSYNC_POSTGRES_POD_NAME="$(kubectl get pods -n qanet -o name --context "$context_name" | grep 'psql-dbsync-cardano-01-db')"
