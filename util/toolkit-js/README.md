...
compact-0.33.0/     → @midnight-ntwrk/compact-js* 2.5.5-rc.4 (vendored .tgz, see below)
```
...
### Variants that vendor an unpublished `compact-js` (e.g. `compact-0.33.0`)

Some `compactc` lines pre-date the publication of their matching `@midnight-ntwrk/compact-js*` and
`@midnight-ntwrk/compact-runtime` packages, so there is nothing on npm to pin. The `compact-0.33.0`
variant (compact-js **2.5.5-rc.4**, for ledger 9) handles this by **vendoring `npm pack` tarballs** referenced
via `file:`:

```
compact-0.33.0/vendor/compact-js.tgz
compact-0.33.0/vendor/compact-js-command.tgz
compact-0.33.0/vendor/compact-js-node.tgz
compact-0.33.0/vendor/compact-runtime.tgz
```
...
the integrity hashes become part of the provenance chain. `compact-runtime` is declared as a
**direct** dependency of the variant (not just transitively) so it hoists to the top level and satisfies
`compact-js`'s range from the blob rather than from the public registry. Everything else in the closure
(`@effect/*`, `platform-js`, `wallet-sdk-address-format`, `ledger-v9`, `onchain-runtime-v4`, …) resolves from
public npm, so the consume path needs **no `.npmrc` and no token**.
...
The GitHub Packages read token is needed **only inside that build** (to pack `compact-runtime` and run the
SDK's install) — never on the consume path. Trust chain: pinned submodule → CI build → bot-signed commit →
