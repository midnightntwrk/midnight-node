...
pub(crate) async fn run_command(cmd: Commands) -> Result<(), Box<dyn std::error::Error>> {
	match cmd {
		Commands::GenerateTxs(args) => generate_txs::execute(args).await.map_err(|e| e.into()),
