...
use crate::genesis_generator::{FundingArgs, GENESIS_NONCE_SEED, GenesisGenerator, IcsConfig};
...
pub async fn execute(
	...
	let cnight_system_tx: Option<SystemTransaction> =
		if let Some(ref filepath) = args.cnight_generates_dust_config {
	...
	let ics_config: Option<IcsConfig> = if let Some(ref filepath) = args.ics_config {
		let json_str = std::fs::read_to_string(filepath)
			.map_err(|e| format!("Failed to read ICS config {:?}: {}", filepath, e))?;
