...
type InnerReturnType = Result<ShowTransactionResult, Box<dyn std::error::Error>>;
...
fn tx_from_bytes(src_file: String, with_context: bool, network: NetworkId) -> InnerReturnType {
	...
	Ok(ShowTransactionResult {
		transaction: if with_context {
			TransactionInfo::TransactionWithContext(deserialize(tx_bytes, network)?)
		} else {
			TransactionInfo::Transaction(deserialize(tx_bytes, network)?)
		},
		size: tx_bytes.len(),
	})
}

fn tx_from_file(src_file: String, with_context: bool, network: NetworkId) -> InnerReturnType {
	let bytes = std::fs::read(&src_file)?;
	Ok(ShowTransactionResult {
		transaction: if with_context {
			TransactionInfo::TransactionWithContext(deserialize(bytes.as_slice(), network)?)
		} else {
			TransactionInfo::Transaction(deserialize(bytes.as_slice(), network)?)
		},
		size: bytes.len(),
	})
}
