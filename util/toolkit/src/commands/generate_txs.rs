...
pub enum GenerateTxsError {
	...
	GetTransactions(Box<dyn std::error::Error>),
	#[error("failed to build transactions: {0}")]
	BuildTransactions(Box<dyn std::error::Error>),
	#[error("failed to build transactions: {0}")]
	SendTransactions(Box<dyn std::error::Error>),
}
...
async fn generate_txs(
	...
	received_txs: DeserializedTransactionsWithContext<Signature, ProofMarker>,
