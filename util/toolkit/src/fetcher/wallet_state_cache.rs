...
/// Deduplicated ledger state snapshot at a specific block height.
///
/// Stored once per (chain_id, block_height) pair, referenced by multiple
/// `CachedWalletState` entries. This eliminates redundant ~49MB copies when
/// multiple wallets share the same chain state.
///
...
/// Each call performs a full topological sort of the storage DAG (~2.6M nodes on a real
/// chain), making it the dominant bottleneck (~20s).
///
...
/// Create a [`LedgerSnapshot`] from a [`LedgerContext`].
///
/// Captures the serialized ledger state, block context, and state root hash.
/// This snapshot is stored once per block height and shared across all wallets.
...
/// Create a [`CachedWalletState`] for a single wallet from a [`LedgerContext`].
...
mod tests {
	...
	/// Benchmark: compare default vs single-pass ledger state serialization.
	///
	/// Requires test fixture: `test-data/ledger_snapshot.zstd` (~119MB).
	/// Run with: `cargo test -p midnight-node-toolkit serialize_benchmark -- --ignored --nocapture`
	#[test]
	#[ignore]
	fn serialize_benchmark() {
		...
		let compressed = std::fs::read(&fixture_path).unwrap_or_else(|e| {
			panic!(
				"Failed to read fixture {fixture_path}: {e}\n\
				 Copy a ledger snapshot to test-data/ledger_snapshot.zstd first."
			)
		});
		...
		println!(
			...
			snapshot.ledger_state_bytes.len(),
			snapshot.ledger_state_bytes.len() as f64 / 1_048_576.0
		);

		let t = std::time::Instant::now();
		let ledger_state = crate::fetcher::trusted_deserialize::trusted_deserialize_tagged::<
			LedgerState<DefaultDB>,
		>(&snapshot.ledger_state_bytes)
		.expect("trusted deserialize failed");
		println!("Deserialize LedgerState: {:?}", t.elapsed());

		// Benchmark A: default serialize (2x topo sort)
		let t = std::time::Instant::now();
		let default_bytes = midnight_node_ledger_helpers::serialize(&ledger_state)
			.expect("default serialize failed");
		let default_time = t.elapsed();
		println!("Default serialize: {:?} ({} bytes)", default_time, default_bytes.len());
		...
		let speedup = default_time.as_secs_f64() / fast_time.as_secs_f64();
