...
pub struct SingleTxBuilder {
	shielded_amount: u128,
	unshielded_amount: u128,
	source_seed: String,
...
impl BuildTxs for SingleTxBuilder {
	...
	async fn build_txs_from(
		...
		let context = LedgerContext::new_from_wallet_seeds(&[funding_seed]);
		context.update_from_txs(received_tx.flat());
...
impl SingleTxBuilder {
	fn build_shielded_offer(
		...
		let offer_fee = Wallet::<DefaultDB>::calculate_fee(1, output_wallets.len() + 1);
		let total_required = offer_fee + amount * output_wallets.len() as u128;
		...
		let remaining_coins = input_amount - offer_fee;
	...
	fn build_unshielded_intents(
		...
		let remaining_nights =
			input_amount - (amount_to_send_per_output * output_wallets.len() as u128);
