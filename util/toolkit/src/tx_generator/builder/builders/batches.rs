...
use crate::{
	Progress, Spin,
	builder::{
		BuildInput, BuildOutput, BuildTxs, BuildUtxoOutput, BuildUtxoSpend, BuilderArgs, DefaultDB,
		DeserializedTransactionsWithContext, DeserializedTransactionsWithContextBatch, FEE_TOKEN,
		FromContext, InputInfo, IntentInfo, LedgerContext, MINT_AMOUNT, NIGHT, OfferInfo,
		OutputInfo, PedersenRandomness, ProofProvider, ProofType, Segment, SignatureType,
		StandardTrasactionInfo, Transaction, TransactionWithContext, UnshieldedOfferInfo,
		UtxoOutputInfo, UtxoSpendInfo, Wallet, WalletSeed,
	},
};

const COIN_AMOUNT: u128 = 100;
...
impl BatchesBuilder {
	...
	fn initial_shielded_offer(
		...
		// Input info
		let input_info =
			InputInfo { origin: funding_seed, token_type: FEE_TOKEN, value: 1_000_000_000_000_000 };
		...
		let mut outputs_info: Vec<Box<dyn BuildOutput<DefaultDB> + Send>> = output_wallets
			...
			.map(|wallet_seed| {
				let output: Box<dyn BuildOutput<DefaultDB> + Send> = Box::new(OutputInfo {
					...
					value: COIN_AMOUNT + total_future_fee_amount,
				});
				output
			})
			.collect();
		...
		let total_coins_required =
			(COIN_AMOUNT + total_future_fee_amount) * self.num_txs_per_batch as u128;
	...
	fn initial_unshielded_intents(
	...
	) -> HashMap<u16, IntentInfo<DefaultDB>> {
		let utxo_spend_info =
			UtxoSpendInfo { value: MINT_AMOUNT, owner: funding_seed, token_type: NIGHT };
		...
		let mut intents: HashMap<u16, IntentInfo<DefaultDB>> = HashMap::new();

		intents.insert(Segment::Fallible.into(), intent_info);
		intents
	}
}
...
impl BuildTxs for BatchesBuilder {
	...
	> {
		...
		// ---------------- UNSHIELDED ------------------------
		let amount_to_send_per_output = MINT_AMOUNT / first_batch_output_wallets.len() as u128;
			...
			> = input_wallet_seeds
				...
				.map(|(index, seed)| {
					...
					let input_info = InputInfo {
						...
						value: COIN_AMOUNT + future_fee_amount,
					};
					...
					let output_info = OutputInfo {
						...
						token_type: FEE_TOKEN,
						value: COIN_AMOUNT + next_future_fee_amount,
					};
