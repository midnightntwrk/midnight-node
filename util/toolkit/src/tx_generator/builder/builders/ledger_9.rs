...
pub fn serialize_tx(
	...
	// `SerializedTx.context` is typed as ledger_8's onchain-runtime BlockContext;
	// re-create it field-by-field from the ledger_9 one (inner Timestamp /
	// HashOutput come from single-versioned base-crypto).
	let v9 = tx.block_context.clone();
	let context = midnight_node_ledger_helpers::ledger_8::BlockContext {
		tblock: v9.tblock,
		tblock_err: v9.tblock_err,
		parent_block_hash: v9.parent_block_hash,
		last_block_time: v9.last_block_time,
	};
