...
use midnight_ledger_unsafe_helpers::ledger_10 as ledger_helpers_local;
...
impl<C: BuilderContext<DefaultDB>> ContractMaintenanceBuilder<C> {
	pub fn new(
		...
		// Each committee member carries its own signature scheme (Schnorr or ledger-10 ECDSA); the
		// pre-ledger-10 ECDSA guard runs earlier via `Builder::relevant_wallet_schemes`.
...
impl<C: BuilderContext<DefaultDB>> BuildTxs for ContractMaintenanceBuilder<C> {
	...
	async fn build_txs_from(
		...
		// looked up per-entrypoint rather than assumed, since from ledger 9 a key can be in
		...
		for p in &self.upsert_entrypoints {
			...
			// The maintenance-update variant is version- (and, from ledger 9, key-format-)
