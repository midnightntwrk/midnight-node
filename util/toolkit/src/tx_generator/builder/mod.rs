...
pub struct BatchesArgs {
	...
	#[arg(short, long, default_value_t = 5_000_000_000_000_000)]
...
pub struct SingleTxArgs {
	...
	#[arg(long)]
	pub shielded_amount: u128,
	/// Amount to send to each unshielded wallet
	#[arg(long)]
	pub unshielded_amount: u128,
...
pub struct DynamicError {
	pub error: Box<dyn std::error::Error + 'static>,
}
...
impl From<Builder> for Box<dyn BuildTxs<Error = DynamicError>> {
	fn from(value: Builder) -> Self {
		...
		match value {
			...
			Builder::ClaimMint(args) => to_builder(ClaimMintBuilder::new(args)),
...
pub trait BuildTxsExt<R> {
	...
	fn context_and_tx_info(
		...
		let context = LedgerContext::new_from_wallet_seeds(&input_wallets_seeds);
...
pub trait IntentToFile: BuildTxsExt<Box<dyn BuildIntent<DefaultDB> + Send>> {
	async fn generate_intent_file(
		...
		tx_info.save_intents_to_file(network_id, dir, partial_name).await;
	}
}
