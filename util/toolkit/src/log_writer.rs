...
fn format_epoch_millis(millis: i64) -> String {
	...
	// Convert days to y/m/d via civil-from-days (algorithm from Howard Hinnant)
	...
	format!("{y:04}-{m:02}-{d:02}T{hours:02}:{minutes:02}:{seconds:02}.{frac_ms:03}Z")
}
