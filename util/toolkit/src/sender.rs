...
use crate::{hash_to_str, serialize};
	...
	<P as ProofKind<DefaultDB>>::Proof: Send + Sync,
{
	pub fn new(network_id: NetworkId, api: OnlineClient<PolkadotConfig>) -> Self {
		Self { network_id, api, _marker_p: PhantomData, _marker_s: PhantomData }
	}

	pub async fn send_tx(
		&self,
		tx: &Transaction<S, P, PedersenRandomness, DefaultDB>,
	) -> Result<(), subxt::Error> {
	...
	pub async fn send_worker(
		...
		for tx in txs {
			...
			let task = tokio::spawn(async move {
				let (tx_hash_string, tx_progress) = self_clone
					.send_tx_no_wait(&tx.clone().into())
					.await
