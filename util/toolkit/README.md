...
Wallet state reconstructed during block replay is cached per seed in `--ledger-state-db` (`MN_LEDGER_CACHE_DB`), so repeat queries only replay new blocks. This works on ledger-8, ledger-9 and ledger-10 chains; a cache written under ledger 8 is discarded (with a warning, falling back to a full replay) once the chain has moved on, while ledger-9 entries resume across the 9 -> 10 fork. During a long replay, `--replay-checkpoint-interval <BLOCKS>` (`MN_REPLAY_CHECKPOINT_INTERVAL`) saves intermediate checkpoints so an interrupted run resumes from the last one instead of genesis.
