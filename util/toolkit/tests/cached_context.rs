...
async fn test_cache_and_restore(backend: &dyn WalletStateCaching, source: &SourceTransactions) {
	...
	let cached = cached.into_ledger9().expect("cached: expected ledger 9");
...
async fn test_split_cached(backend: &dyn WalletStateCaching, source: &SourceTransactions) {
	...
	let cached = cached_ctx.into_ledger9().expect("cached: expected ledger 9");
	let raw = raw_ctx.into_ledger9().expect("raw: expected ledger 9");
