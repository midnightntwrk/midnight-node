...
async fn find_utxo_by_tx_id(client: &OgmiosClients, address: &str, tx_id: &str) -> Option<OgmiosUtxo> {
	let mut attempts = 0;
	while attempts < 10 {
...
async fn get_ogmios_client(cfg: &AppConfig) -> OgmiosClients {
	let ogmios_url = cfg.ogmios_url.clone();
	client_for_url(&ogmios_url, Duration::from_secs(5)).await.unwrap()
}

fn build_asset_vector(ogmios_utxo: &OgmiosUtxo) -> Vec<Asset> {
...
async fn send(address: &str, assets: Vec<Asset>) -> String {
	// Load config
	let cfg = load_config();
	let payment_addr = cfg.payment_addr.clone();
	let client = get_ogmios_client(&cfg).await;
	...
	// Parse the signing key from cborHex in JSON
...
async fn register(cardano_address_hex: &str, midnight_address_hex: &str, signing_wallet: &Wallet, tx_in: &OgmiosUtxo, collateral_utxo: &OgmiosUtxo) {
	...
	let datum = serde_json::to_string(&serde_json::json!({
                        "constructor": 0,
                        "fields": [{ "bytes": cardano_address_hex }, { "bytes": midnight_address_hex }]
                    })).unwrap();
...
fn get_wallet_address(wallet: &Wallet) -> String {
...
async fn make_collateral(address: &str) -> OgmiosUtxo {
...
async fn send_money() {
	...
	let cardano_address_hex = Address::from_bech32(&cfg.payment_addr).unwrap().to_hex();

	register(&cardano_address_hex, &dust_hex, &wallet, &tx_in, &collateral_utxo).await;
}
