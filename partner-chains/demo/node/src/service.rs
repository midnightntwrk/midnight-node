...
use sp_partner_chains_consensus::PartnerChainsProposerFactory;
...
/// [`sp_partner_chains_consensus::SlotExtractor`] reading the slot from the Aura
/// pre-runtime digest.
pub struct AuraSlotExtractor;

impl sp_partner_chains_consensus::SlotExtractor<Block> for AuraSlotExtractor {
...
> {
	...
	// before delegating, so the Aura verifier's own inherent check recreates the Partner Chains
	// inherents through this same factory instead of rejecting them.
